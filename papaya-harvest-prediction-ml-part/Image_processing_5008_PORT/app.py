...
import timm
from suggetions import generate_stage_guidance
...
def predict():
    ...
    if passed_threshold:
        ...
        # ─────────────────────────────
        # STEP 3 — GROWTH STAGE GUIDANCE
        # ─────────────────────────────
        try:
            location = request.form.get("location", "Sri Lanka")
            current_month = request.form.get("month", None)
            
            stage_guidance = generate_stage_guidance(
                stage_code=GRADE_CLASSES[gidx],
                location=location,
                current_month=current_month
            )
            response["growth_guidance"] = stage_guidance
        except Exception as e:
            response["growth_guidance"] = {
